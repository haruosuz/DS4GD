...
`write.fasta()`関数を用いて、配列データをFASTA形式ファイルとして書き出す:  
    ...
    write.fasta(sequences = ld[i], names = getAnnot(ld[i]), file.out = "mySequences.fasta")
...
多重配列アライメント (多重整列)
    ...
    library(Biostrings)
    mySeq <- readAAStringSet(file = "mySequences.fasta")
    ...
    # 多重配列アライメントをファイル出力
    writeXStringSet(unmasked(myAln), file="myAln.fasta")
...
系統解析 
    ...
    myAln <- read.aa(file = "myAln.fasta", format = "fasta")

    # 配列間の距離を計算
    d <- dist.ml(myAln, model="Dayhoff")
    ...
    # 系統樹をnewick形式でファイル出力
    write.tree(tre, file="myTree.newick")

...
"harakiri"、納豆またはキムチ "(natto|kimchi)"、[ベロ毒素](https://ja.wikipedia.org/wiki/ベロ毒素)([Shiga toxin](https://en.wikipedia.org/wiki/Shiga_toxin))にマッチする要素番号を取得する:  

    # grep(pattern, x) returns the positions of all elements in x that match pattern
    i <- grep(pattern = "harakiri", x = annotation, ignore.case = TRUE)
    i <- grep(pattern = "(natto|kimchi)", x = annotation, ignore.case = TRUE)
    i <- grep(pattern = "Shiga.* toxin", x = annotation, ignore.case = TRUE)
    length(i)
...
`write.fasta()`関数を用いて、配列データをFASTA形式ファイルとして書き出す:  
    ...
    write.fasta(sequences = ld[i], names = getAnnot(ld[i]), file.out = "myseq.fasta")
...
作業を中断し再開する（Rを終了し再起動する）。作業ディレクトリを変更し、パッケージ`seqinr`を呼び出し、`read.fasta()`関数で配列データを読み込む:  
    ...
    lx <- read.fasta(file = "myseq.fasta", seqtype = c("AA"), strip.desc = TRUE)    # Reading sequence data
...
2007/11/08 [アミノ酸組成を利用した医薬分子結合部位の予測](https://www.jstage.jst.go.jp/article/ciqs/2007/0/2007_0_J07/_article/-char/ja/)

タンパク質の細胞内局在予測 ~ モチーフ表現を軸に ~ ftp://ftp.hgc.jp/pub/hgc/db/knakai/titech2008-2.pdf June 19, 2008 中井謙太 

[WoLF PSORT](https://wolfpsort.hgc.jp/aboutWoLF_PSORT.html.ja) はアミノ酸配列から蛋白質の細胞内局在部位を予測する。

2009-09-03 [SOSUIとTMHMMを使って膜タンパク質の膜貫通領域を予測する 統合TV](http://togotv.dbcls.jp/20090903.html)

![http://motdb.dbcls.jp/?AJACS10%2Fneta%2Fmotif](http://motdb.dbcls.jp/?plugin=ref&page=AJACS10%2Fneta%2Fmotif&src=AJACS10.024.jpg)
...
[Pairwise global alignment of DNA sequences](https://github.com/haruosuz/r4bioinfo/tree/master/R_Avril_Coghlan#pairwise-global-alignment-of-protein-sequences-using-the-needleman-wunsch-algorithm)  

...
"harakiri"、
[ベロ毒素](https://ja.wikipedia.org/wiki/ベロ毒素)([Shiga toxin](https://en.wikipedia.org/wiki/Shiga_toxin))
にマッチする要素番号を取得する:  

    # grep(pattern, x) returns the positions of all elements in x that match pattern
    i <- grep(pattern = "harakiri", x = myAnnotation, ignore.case = TRUE)
    i <- grep(pattern = "Hemoglobin.*(alpha|beta).*OS=(Gorilla|Homo sapiens|Mus musculus|Rattus|Xenopus laevis)", x = myAnnotation, ignore.case = TRUE)
    length(i)
...
[SeaView](http://www2.tba.t-com.ne.jp/nakada/takashi/phylogeny/seaview2.html)で多重配列アライメントを表示する。
...
![](http://blog.amelieff.jp/images/weblogo2.png)
    ...
    plot.phylo(mytree, type = "phylogram") # type = "phylogram", "cladogram", "fan", "unrooted", "radial"

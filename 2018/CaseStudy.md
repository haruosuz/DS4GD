...
データのインポート。`read.delim()`関数でタブ区切りファイルを読み込む:  
    ...
    d <- read.delim(file = basename(curl), stringsAsFactors = FALSE, skip = 1, check.names = FALSE) 
...
パッケージを用いて、配列データを取得:  
    ...
    curl <- "ftp://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/000/008/865/GCF_000008865.1_ASM886v1/GCF_000008865.1_ASM886v1_genomic.fna.gz"
